...
kable_tex_file = sprintf('%sbigtable_s.tex', output_dir)

#### dashboard ####

all_data_names = c("nba_coach_2021", "survivor", "law_modified", "csrankings", "sushi", "airlines", "btl", "btl_modified", "dices350")
...
METRIC_TITLES = c(
    ...
    'mis_delta_inversions_med_pairwise' = "$\\Delta$ Sampling",
    'adv_delta_inversions_max_pairwise' = "$\\Delta$-Gaming",
    
    
    # Baseline (Bsl) metrics (Unused)
    'bsl_missing_pairwise' = "Missing Pairwise",
    'bsl_med_disagreement_per_judge_pairwise' = "Median Disagreement Per Judge",
    ...
    'bsl_disagreement_per_judge_range' = "Median Disagreement Per User",
    ...
    'adv_delta_specificity_med_cnt' = "$\\Delta$ Specifications Med (Adversarial)",
...
dissent_analysis <- raw_df %>%
    filter(method_name == "sra", metric_name %in% c('n_abstentions_pairwise')) %>%  # Fixed parentheses for %in%
...
dissent_df <- raw_df %>%
    ...
    rename(dissent_min = dissent_first_uni) 


processed_sra_df <- raw_df %>%
    filter(method_name == "sra") %>%
    left_join(dissent_df, by = c("data_name")) %>%
    rename(method_name = method_name.x) %>% 
    select(-method_name.y) %>%
    mutate(dissent_rate = as.numeric(dissent_rate)) %>%
...
table_results_df = raw_results_df %>%
    ...
    mutate(
        svalue = case_when(
            ...
            str_ends(stat_name, "_dec") ~ sprintf("%1.3f", stat_value),
        ...
        svalue = ifelse(is.na(stat_value) & stat_name == 'bsl_dissent_rate_dec', "\\navalue{}", svalue),
        svalue = ifelse(is.na(stat_value) & stat_name != 'bsl_dissent_rate_dec', "", svalue),
        svalue = ifelse(!str_detect(method_name, "^sra") & stat_name == 'bsl_n_abstentions_pairwise', "\\navalue{}", svalue)
    ) %>%
    mutate(
        svalue = case_when(
            stat_name %in% c("mis_delta_inversions_med_pairwise", "adv_delta_inversions_max_pairwise") & svalue == "0.0\\%" ~ paste0("\\goodvalue{", svalue, "}"),
            stat_name %in% c("mis_delta_inversions_med_pairwise", "adv_delta_inversions_max_pairwise") & svalue != "0.0\\%" ~ paste0("\\badvalue{", svalue, "}"),
            TRUE ~ svalue
        )
    ) %>%
    select(-stat_value) %>%
...
headers = headers_df %>%
    ...
    pull(method_name) %>%
    prepend(c("\\textheader{Dataset}", "\\textheader{Metrics}"))
