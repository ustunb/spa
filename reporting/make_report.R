...
# Set the correct path to Pandoc
Sys.setenv(RSTUDIO_PANDOC="/Applications/RStudio.app/Contents/Resources/app/quarto/bin/tools")
options(tinytex.tlmgr.path = tinytex:::find_tlmgr("/Users/Kadekool/.virtualenvs/reticulate-python-env/TinyTex"))

# Function to ensure directory paths end with a slash
...
p = add_argument(p, "input", type = "character", help = "name of template file")
p = add_argument(p, "report_data", type = "character", help = "name of report data file")
p = add_argument(p, "--output_format", type = "character", help = "format", default = "all")
p = add_argument(p, "--output_file", type = "character", help = "name of output file")
p = add_argument(p, "--output_dir", type = "character", help = "directory for rendered output file")
p = add_argument(p, "--report_python_dir", type = "character", help = "directory for python")
p = add_argument(p, "--build_dir", type = "character", help = "directory where to knit the document")
p = add_argument(p, "--venv_dir", type = "character", help = "directory for the virtual environment for reticulate")
p = add_argument(p, "--output_options", help = "list of output options (overrides options in metadata)")
p = add_argument(p, "--output_yaml", type = "character", help = "path to YAML file(s) specifying output format")
p = add_argument(p, "--runtime", default = "static", type = "character", help = "Runtime target for rendering. Options include: 'auto', which chooses the target based on YAML metadata; 'shiny' to render a Shiny file; 'static' to render a static file. The default is 'auto' or 'static' if there is no YAML metadata.")
p = add_argument(p, "--params", help = "list of named params  (override params in the YAML front-matter)")
p = add_argument(p, "--template_file", type = "character", help = "name of template file")
p = add_argument(p, "--data_name", type = "character", help = "data name")
# Flags
p = add_argument(p, "--clean", flag = TRUE, help = "flag to clean files after rendering")
p = add_argument(p, "--quiet", flag = TRUE, help = "flag to suppress printing the pandoc command line")
p = add_argument(p, "--run_pandoc", flag = TRUE, help = "flag to run pandoc to convert Markdown output")

argv <- parse_args(p)
# Other render arguments are ignored because they are fixed/unused

# Load required packages
required_packages = c('tools', 'rmarkdown', 'knitr', 'reticulate')
...
use_python(argv$venv_dir, required = TRUE)
